...
Here's how you can see where you are (on BCE it will be the home directory for the *ubuntu* user, where files specific to that user are stored)...
```{r, engine='bash', pwd1}
pwd
```
...
The filesystem is basically a upside-down tree. The top, or root of the tree, is the `/` directory. Within `/` there are subdirectories, such as `/home` (which contains users' home directories where all of the files owned by a user are stored) and `/bin` (containing UNIX programs, aka 'binaries'). For the *ubuntu* user, everything specific to that user is stored in `/home/ubuntu`. If you are the *ubuntu* user, you can also refer to `/home/ubuntu` by the shortcuts `~` and `~ubuntu`. 
...
cp unit8-bigData.R unit8-bigData-new.R
mv unit8-bigData-new.R /tmp/.
cd /tmp
ls -lrt 
```
```{r, cp3}
cd /tmp
rm unit8-bigData-new.R
ls -lrt
```
...
```{r, mv}
cd ~/stat243-fall-2015/units
mv unit8-bigData.R unit9-bigData-processing.R
ls
```
...
```{r, scp, eval=FALSE}
cd ~/stat243-fall-2015/units
# copy the file to Desktop subdirectory of the scf1 home directory on the remote machine:
...
Now let's focus on the 10 characters in the first column. Items marked with a `d` as the first character directories. Following that first character are three triplets of file permission information. Each triplet contains read ('r'), write ('w') and execute ('x') information. The first `rwx` triplet indicates if the owner of the file can read, write, and execute a file. The second `rwx` triplet indicates if anyone in the group that the owner belongs to can read, write and execute a file. The third triplet pertains to any user. Dashes mean that a given user does not have that kind of access to the given file.
...
If we only want a summary of usage for each subdirectory, rather than showing all files in all subdirectories:
...
3) Change the permissions on the file stat243-fall-2015/units/unit1-unix.sh to be readable by the user and the group, writeable by the user, and executable by all users.
...
7) Where is Python installed on the system? What are some other programs/executables that are installed in the same directory.



```{r, cleanup, echo=FALSE}
if [ -f ~/stat243-fall-2015/data/coop.txt.gz ]; then gunzip ~/stat243-fall-2015/data/coop.txt.gz 
fi
