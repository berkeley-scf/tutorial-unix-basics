...
A [screencast](http://youtu.be/pAY6E0FdWUo) of this material accompanies this document and the end of this document has questions that test your understanding of the material. Note that the screencast was based on an earlier version of this material (the version tagged 'version1' in the Github repository). It will not exactly match what is seen here, but it should hopefully be close enough to still be helpful. Furthermore the screencast uses an outdated virtual Linux machine. You should be able to follow the screencast and replicate what is demonstrated in the screencast from other UNIX-like environments, such as the Terminal on a Mac or the Windows Subsystem for Linux. 

I've generated this document based on using the bash shell on an Ubuntu Linux version 20.04 machine, but you should be able to replicate most of the steps in this tutorial in other UNIX-like environments, in particular when using the bash or zsh shells. (zsh is the default shell on recent MacOS versions.)
...
If you'd like to work in a Terminal window on your own Mac, you'll find the Terminal under `Applications -> Utilities -> Terminal`). On Windows, if you have a sufficiently new version of Windows 10, you can use the [Windows Subsystem for Linux](https://msdn.microsoft.com/en-us/commandline/wsl/install_guide), which will provide you with an Ubuntu shell running bash. If you have access to a JupyterHub, you will likely be able to start a Terminal session under "New". You could also try a cloud service such as Google [Cloud Shell](shell.cloud.google.com). Note that you probably shouldn't use `Git Bash` as its functionality is limited. 
...
Next let's clone a more extensive repository - we'll get the materials for my Statistics 243 class from Fall 2020.  
...
cd stat243-fall-2020 # go to subdirectory based on relative path
pwd
```
```{r cd2}
cd /accounts/gen/vis/paciorek/stat243-fall-2020 # absolute path to the same location
pwd
```
```{r cd3}
cd /accounts/gen/vis/paciorek/stat243-fall-2020
cd ..  # go up a directory to the parent of the current directory
pwd
```
```{r cd5}
cd /accounts/gen/vis/paciorek/stat243-fall-2020/units
...
```{r, perm}
cd ~/stat243-fall-2020
echo "first line" > tmp.txt  # create a text file we can play with
ls -l
```
...
E.g., for the *syllabus.pdf* file, all users can read the file but only the owner can modify the file by writing to it. 
...
Essentially all UNIX commands/utilities have (often very extensive and hard to parse) help information (called a man page), accessed using `man`.
...
The shell is the UNIX program that you have been interacting with when using the Terminal. The shell sits between you and the operating system and provides useful commands and functionality. There are actually different shells that you can use, of which `bash` is very common and is the default on many systems. You've been using `bash` without knowing it.
