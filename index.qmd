...
```{bash}
rm -r /tmp/test/assets   # Remove the ps directory and anything contained within it.
...
```{bash}
gzip test.png
ls -l test.png.gz   # This is rather smaller than the uncompressed file.
```
...
Here we see that the total usage is about 665 KB, with, for example, 74 KB of that in the `img` subdirectory.
...
You'll generally want to have the first line of your shell scripts indicate the shell to be used to execute the script, e.g., putting `#!/bin/bash` as we did above for bash scripts.
